...
def smart_keyboard(item):
    d=item['parsed']; rows=[]
    # Only show choices that exist for the current catalog context.
    if not d.get('edition'):
        opts=all_context_options('TIPO_EDICION',d)
        if opts: rows.append([cb(x,f'set:edition:{x}') for x in opts[:4]])
    if not d.get('size'):
        sk=size_keyboard(d)
        if sk: rows.extend(sk.inline_keyboard)
    if d.get('edition') and norm(d.get('edition'))=='player edition' and not item.get('custom_answered') and not item.get('requested_name'):
        rows.append([cb('✏️ Sí, poner nombre/número','custom:yes'),cb('Sin nombre','custom:no')])
    if not d.get('patch'):
        opts=all_context_options('PARCHE',d)
        if opts: rows.append([cb(x,f'set:patch:{x}') for x in opts[:4]])
    if not d.get('model'):
        opts=all_context_options('MODELO',d)
        if opts: rows.append([cb(x,f'set:model:{x}') for x in opts[:4]])
    rows.append([cb('➖ 1', 'set:qty:1'),cb('➕ 2','set:qty:2'),cb('➕ 3','set:qty:3')])
    rows.append([cb('✅ Confirmar pedido','confirm'),cb('❌ Cancelar artículo','cancel')])
...
async def close_order(update,context):
    ...
    with open(path,'rb') as f: await update.message.reply_document(document=f,filename=os.path.basename(path),caption=f'📄 PEDIDO_PROVEEDOR_{state["id"]:04d}.xlsx — revisa el archivo y usa «reabrir pedido» si necesitas corregirlo.')
...
async def handle_photo(update,context):
    ...
    if rs:
        await update.message.reply_text('📸 Foto recibida. Estas son las coincidencias del CATALOGO:',reply_markup=candidate_keyboard(rs))
    else:
...
async def process_text_for_pending(update,text):
    ...
    if rs: await update.message.reply_text('🔎 Actualicé la búsqueda. Selecciona el producto correcto:',reply_markup=candidate_keyboard(rs))
...
async def text_item(update,context,text):
    ...
    if rs: await update.message.reply_text('🔎 Encontré estas opciones en el CATALOGO. Selecciona una:',reply_markup=candidate_keyboard(rs))
...
async def callbacks(update,context):
    ...
    if data.startswith('set:'):
        ...
        elif field=='qty': p['quantity']=int(val); p['parsed']['quantity']=int(val)
        ...
        if p.get('catalog'):
            ...
            if norm(p['catalog'].get('TIPO_EDICION'))=='player edition' and not p.get('custom_answered') and not p.get('requested_name'):
                await q.edit_message_text(item_summary(p)+'\n\n¿Quieres poner nombre y número?',reply_markup=smart_keyboard(p)); return
            await q.edit_message_text(item_summary(p)+'\n\nConfirma el artículo:',reply_markup=kb_confirm(p)); return
        await q.edit_message_reply_markup(reply_markup=smart_keyboard(p)); return
    if data=='confirm':
