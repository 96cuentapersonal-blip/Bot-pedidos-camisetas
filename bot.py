import os, re, json, sqlite3, shutil, threading, urllib.request, urllib.error, base64, tempfile
...
EXCEL_NAME = os.environ.get('EXCEL_FILE', 'SISTEMA_PEDIDOS_SERGIO_V8.xlsx')
EXCEL_FILE = Path(EXCEL_NAME)
if not EXCEL_FILE.is_absolute():
    EXCEL_FILE = BASE_DIR / EXCEL_FILE
DB_NAME = os.environ.get('DB_FILE', 'bot_v8.sqlite3')
...
# V8: vision + catalog + official roster order assistant.
...
def roster_match(team, query):
    ...
    # exact / containment / token overlap
    for num,name in rr:
        if norm(name)==q: return (num,name)
    for num,name in rr:
        if q in norm(name) or norm(name) in q: return (num,name)
    return None
...
def vision_identify(image_path, caption=''):
    ...
    prompt={
      ...
      'caption':caption,
      'schema':{'team':'string|null','season':'string|null','model':'Local|Visitante|Tercera|null','edition':'Player Edition|Fan Edition|Retro|null','category':'string|null','patch':'Liga|Champions|null','confidence':'number','notes':'string'}
    }
    body={
      'model':VISION_MODEL,
...
def extract_name_number(text, team=None):
    raw=text or ''; name=None; number=None
    ...
    if not name:
        # Roster is the preferred source for player mode.
        rr=roster_for(team)
        low=norm(raw)
        for num,pname in rr:
            if norm(pname) in low:
                name=pname; number=number or num; break
    if not name:
        for r in catalog_rows():
            player=r.get('JUGADOR')
            if player and norm(player) in norm(raw): name=str(player); break
    # Compact name + number, e.g. "Gabriel Jesus 9". Prefer roster canonical spelling.
    if not name:
        m=re.match(r'^\s*([A-Za-zÀ-ÿ][A-Za-zÀ-ÿ .\'-]{1,35}?)\s+#?(\d{1,2})\s*$',raw)
        if m:
    ...
    if name and team:
        ...
        if hit:
            # In Player mode this gives canonical spelling and official number; caller decides whether to validate.
            name=hit[1]
...
def parse(text, team=None):
    ...
    for r in catalog_rows():
        row_team=r.get('EQUIPO')
        if row_team and norm(row_team) in t: d['team']=str(row_team)
    name,number=extract_name_number(text, team or d.get('team'))
...
async def start(update,context):
    await update.message.reply_text('🤖 Bot V8 listo.\n\nEscribe «nuevo pedido» y luego envía fotos o descripciones. La foto se analiza con IA cuando OPENAI_API_KEY está configurada; después el CATALOGO determina el ID del proveedor.')
...
async def new_order(update,context):
    ...
    state={'id':next_id(),'status':'ABIERTO','items':[],'pending':None,'awaiting_close':False}; save_state(chat,state)
...
async def close_order(update,context):
    ...
    state['awaiting_close']=True; save_state(chat,state)
    units=sum(int(x.get('quantity') or 1) for x in state['items'])
    await update.message.reply_text(f'🔒 ¿Confirmas cerrar el pedido #{state["id"]:04d}?\n📦 Artículos: {len(state["items"])}\n🔢 Unidades: {units}', reply_markup=InlineKeyboardMarkup([[cb('✅ Sí, cerrar','confirm_close'),cb('↩️ Seguir editando','cancel_close')]]))
    return

async def finalize_close(chat, state, message):
    set_order_status(state['id'],'CERRADO'); state['status']='CERRADO'; state['awaiting_close']=False; save_state(chat,state)
    try: path=rebuild_output(state['id'])
    except Exception as e:
        await message.reply_text(f'⚠️ Cerré el pedido, pero no pude generar el Excel: {e}'); return
    units=sum(int(x.get('quantity') or 1) for x in state['items'])
    await message.reply_text(f'🔒 Pedido #{state["id"]:04d} cerrado.\n📦 Artículos: {len(state["items"])}\n🔢 Unidades: {units}\n📄 Enviando Excel final…')
    with open(path,'rb') as f: await message.reply_document(document=f,filename=os.path.basename(path),caption=f'📄 PEDIDO_PROVEEDOR_{state["id"]:04d}.xlsx — pedido cerrado.')
    await message.reply_text('¿Qué quieres hacer ahora?', reply_markup=InlineKeyboardMarkup([[cb('📋 Revisar pedido','review'),cb('🆕 Siguiente pedido','new_order_btn')]]))

async def reopen(update,context):
    chat=update.effective_chat.id; state=load_state(chat)
    if not state: await update.message.reply_text('No hay pedido para reabrir.'); return
    state['status']='ABIERTO'; state['pending']=None; state['awaiting_close']=False; save_state(chat,state)
...
async def handle_photo(update,context):
    ...
    p={'photo_file_id':photo.file_id,'caption':caption,'text':caption,'parsed':parse(caption),'catalog':None,'quantity':1,'confidence':0}
    # Download Telegram photo and send it to the vision model.
    tmp_path=None
    try:
        tgfile=await context.bot.get_file(photo.file_id)
        fd,tmp_path=tempfile.mkstemp(suffix='.jpg'); os.close(fd)
        await tgfile.download_to_drive(custom_path=tmp_path)
        vis,err=vision_identify(tmp_path,caption)
    except Exception as e:
        vis,err=None,str(e)
    finally:
        pass
    if vis:
        for k in ('team','season','model','edition','category','patch'):
            if vis.get(k): p['parsed'][k]=vis[k]
        try: p['confidence']=float(vis.get('confidence') or 0)
        except Exception: p['confidence']=0
        p['vision']=vis
        # Visual result is used to select the supplier catalog row; it does not invent IDs.
        rs=candidates(p.get('text','')+' '+ ' '.join(str(vis.get(k) or '') for k in ('team','season','model','edition','category','patch')))
        # Strongly constrain by visual team/model/edition when supplied.
        vd=p['parsed']
        filtered=[]
        for r in catalog_rows():
            if not active(r): continue
            if vd.get('team') and norm(r.get('EQUIPO'))!=norm(vd['team']): continue
            if vd.get('model') and norm(r.get('MODELO'))!=norm(vd['model']): continue
            if vd.get('edition') and norm(r.get('TIPO_EDICION'))!=norm(vd['edition']): continue
            if vd.get('season') and norm(r.get('TEMPORADA'))!=norm(vd['season']): continue
            if vd.get('patch') and norm(r.get('PARCHE'))!=norm(vd['patch']): continue
            if vd.get('sleeve') and norm(r.get('MANGA'))!=norm(vd['sleeve']): continue
            filtered.append((score_row(r,vd,p.get('text','')),r))
        if filtered:
            filtered.sort(key=lambda x:x[0],reverse=True)
            # Collapse duplicate IDs so one supplier product is never shown twice.
            unique=[]; seen=set()
            for score,row in filtered:
                rid=str(row.get('ID_PRODUCTO') or '')
                if rid and rid not in seen:
                    unique.append((score,row)); seen.add(rid)
            filtered=unique
            p['candidates']=[r for _,r in filtered]
            # High-confidence vision auto-selects a clear catalog match. Never force 1/2/3 selection.
            if p['confidence']>=85 and len(filtered)==1:
                p['catalog']=filtered[0][1]
            elif p['confidence']>=85 and len(filtered)>1:
                top=filtered[0][0]; second=filtered[1][0]
                if top-second>=12: p['catalog']=filtered[0][1]
        else:
            p['candidates']=[r for _,r in rs]
    else:
        p['candidates']=candidates(caption)
    if tmp_path and os.path.exists(tmp_path):
        try: os.unlink(tmp_path)
        except Exception: pass
    state['pending']=p; save_state(update.effective_chat.id,state)
    if p.get('catalog'):
        c=p['catalog']; d=p['parsed']
        conf=int(round(p.get('confidence',0)))
        msg=(f'📸 **Imagen identificada**\n\n⚽ {c.get("EQUIPO","")}\n'
             f'📅 {c.get("TEMPORADA","")}\n👕 {c.get("TIPO_EDICION","")}\n'
             f'🏠 {c.get("MODELO","")}\n🆔 {c.get("ID_PRODUCTO","")}\n'
             f'🎯 Confianza visual: {conf}%')
        if not d.get('size') or not d.get('patch') or (norm(c.get('TIPO_EDICION'))=='player edition' and not p.get('player_selected') and not p.get('custom_answered')):
            await update.message.reply_text(msg+'\n\nCompletemos los datos:',reply_markup=smart_keyboard(p),parse_mode='Markdown')
        else:
            await update.message.reply_text(item_summary(p)+'\n\nConfirma el artículo:',reply_markup=kb_confirm(p))
        return
    if p.get('candidates'):
        # Ambiguous images are handled by targeted attributes, not candidate 1/2/3 buttons.
        await update.message.reply_text('📸 Analicé la foto, pero no tengo suficiente confianza para elegir un ID automáticamente. Completa los datos que faltan:',reply_markup=smart_keyboard(p))
    elif vis:
        team=vis.get('team') or 'equipo no determinado'
        await update.message.reply_text(f'📸 Reconocí la camiseta como **{team}**, pero ese producto todavía no tiene un ID en tu CATALOGO. No voy a inventarlo.\n\nCompleta los datos o agrega el producto al Excel.',parse_mode='Markdown',reply_markup=smart_keyboard(p))
    else:
        await update.message.reply_text('📸 No pude analizar visualmente la foto. '+('Configura OPENAI_API_KEY en Render para activar la identificación por IA.' if not OPENAI_API_KEY else f'Error de visión: {err}'),reply_markup=smart_keyboard(p))
...
async def process_text_for_pending(update,text):
    ...
    if p.get('awaiting')=='player_mismatch':
        await update.message.reply_text('Selecciona el jugador oficial corregido o pulsa Personalizado.', reply_markup=InlineKeyboardMarkup([[cb(f'⚽ {p.get("mismatch_name")} #{p.get("mismatch_official_number")}', 'playerfix'), cb('✏️ Personalizado','custom:yes')]])); return True
    if p.get('awaiting')=='custom_name':
        name,number=extract_name_number(text, (p.get('parsed') or {}).get('team') or ((p.get('catalog') or {}).get('EQUIPO')))
        if not name:
    ...
    p['text']=(p.get('text','')+' '+text).strip(); p['parsed']=parse(p['text'], (p.get('parsed') or {}).get('team') or ((p.get('catalog') or {}).get('EQUIPO')))
    if p.get('catalog'):
        d=p['parsed']
        ...
        if d.get('name') and not p.get('requested_name'):
            team=(p.get('parsed') or {}).get('team') or p.get('catalog',{}).get('EQUIPO')
            official_hit=roster_match(team,d.get('name'))
            if official_hit:
                official_num,official_name=official_hit
                requested_num=d.get('number')
                if requested_num and requested_num!=official_num:
                    p['awaiting']='player_mismatch'; p['mismatch_name']=official_name; p['mismatch_official_number']=official_num; p['mismatch_requested_number']=requested_num
                    save_state(update.effective_chat.id,state)
                    await update.message.reply_text(f'⚠️ {official_name} corresponde al número {official_num}, no al {requested_num}.\n\nSi quieres la camiseta oficial, selecciona {official_name} #{official_num}. Si quieres el nombre/número libre, usa Personalizado.', reply_markup=InlineKeyboardMarkup([[cb(f'⚽ {official_name} #{official_num}','playerfix'),cb('✏️ Personalizado','custom:yes')]]))
                    return True
                p['requested_name']=official_name; p['requested_number']=requested_num or official_num; p['player_selected']=True; p['custom_answered']=True
            else:
                ...
                if norm(d['name'])!=official:
                    p['awaiting']='custom_confirm'; save_state(update.effective_chat.id,state)
                    await update.message.reply_text(f'⚠️ «{d["name"]}» no figura como jugador oficial de este equipo. ¿Quieres usarlo como personalización?', reply_markup=InlineKeyboardMarkup([[cb('✅ Sí, personalizar','custom:yes'),cb('❌ No','custom:no')]]))
                    return True
                p['requested_name']=d['name']; p['requested_number']=d.get('number') or parse_number(p['catalog'].get('NUMERO'))
        if not d.get('size'):
...
async def callbacks(update,context):
    ...
    if data=='playerfix':
        ...
        save_state(q.message.chat.id,state)
    ...
    if data=='new_order_btn':
        ...
        ns={'id':next_id(),'status':'ABIERTO','items':[],'pending':None,'awaiting_close':False}; save_state(q.message.chat.id,ns)
    ...
    if data=='close_now':
        ...
        state['awaiting_close']=True; save_state(q.message.chat.id,state)
        units=sum(int(x.get('quantity') or 1) for x in state['items'])
        await q.message.reply_text(f'🔒 ¿Confirmas cerrar el pedido #{state["id"]:04d}?\n📦 Artículos: {len(state["items"])}\n🔢 Unidades: {units}', reply_markup=InlineKeyboardMarkup([[cb('✅ Sí, cerrar','confirm_close'),cb('↩️ Seguir editando','cancel_close')]])); return
    if data=='confirm_close':
        if not state.get('items'):
            await q.message.reply_text('⚠️ El pedido está vacío.'); return
        await finalize_close(q.message.chat.id,state,q.message); return
    if data=='cancel_close':
        state['awaiting_close']=False; save_state(q.message.chat.id,state)
        await q.message.reply_text('↩️ Pedido sigue abierto. Puedes continuar agregando o corrigiendo artículos.'); return
    if data=='review':
        lines=[f'📦 Pedido #{state["id"]:04d} — {state["status"]}','']
        for i,x in enumerate(state['items'],1):
        ...
        units=sum(int(x.get('quantity') or 1) for x in state['items'])
        lines.append(f'\n📦 Artículos: {len(state["items"])} · 🔢 Unidades: {units}')
        lines.append('Cuando termines escribe «cierro pedido».')
...
async def ping(update,context): await update.message.reply_text(f'V8 activo. Vision: {"OK" if OPENAI_API_KEY else "FALTA OPENAI_API_KEY"}. Excel: {"OK" if EXCEL_FILE.exists() else "FALTA"}.')
...
def health():
    ...
    return f'Bot V8 activo - Excel: {"OK" if exists else "FALTA"} - productos: {n} - vision: {"OK" if OPENAI_API_KEY else "FALTA OPENAI_API_KEY"}',200 if exists else 503
