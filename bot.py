...
# V8.1: real image vision + catalog matching + stable pending-item text corrections + roster validation.
...
async def ping(update,context): await update.message.reply_text(f'V8.1 activo. Vision: {"OK" if OPENAI_API_KEY else "FALTA OPENAI_API_KEY"} ({VISION_MODEL}). Excel: {"OK" if EXCEL_FILE.exists() else "FALTA"}.')
