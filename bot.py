...
EXCEL_NAME = os.environ.get('EXCEL_FILE', 'SISTEMA_PEDIDOS_SERGIO_V8_1.xlsx')
EXCEL_FILE = Path(EXCEL_NAME)
if not EXCEL_FILE.is_absolute():
    EXCEL_FILE = BASE_DIR / EXCEL_FILE
DB_NAME = os.environ.get('DB_FILE', 'bot_v8_1.sqlite3')
...
# V8.3: real image vision + catalog matching + stable pending-item corrections + patch-safe catalog flow.
...
def extract_name_number(text, team=None):
    ...
    # Explicit number first.
    m=re.search(r'(?:numero|número|num|#)\s*[:=\-]?\s*(\d{1,2})\b', raw, re.I)
    if m: number=m.group(1)
    rr=roster_for(team) if team else []
    low=norm(raw)
    # Prefer official roster names. This also handles "Yamal 10" naturally.
    for num,pname in rr:
        if norm(pname) in low:
            name=pname
            if number is None: number=num
            break
    if not name:
        # Catalog player names are a second source of canonical spelling.
        for r in catalog_rows():
            player=r.get('JUGADOR')
            if player and norm(player) in low:
                name=str(player)
                if number is None and r.get('NUMERO') not in (None,''):
                    number=parse_number(r.get('NUMERO'))
                break
    if not name:
    ...
    if not name:
        # Compact free-form: "Sergio 10" / "Yamal #10".
...
async def handle_photo(update,context):
    ...
    if vis:
        p['vision']=vis
        for k in ('team','season','model','edition','category','patch','sleeve'):
    ...
    state['pending']=p; save_state(chat,state)
    ...
    if p.get('catalog'):
        ...
        if not p['parsed'].get('size') or (norm(c.get('TIPO_EDICION'))=='player edition' and not p.get('requested_name') and not p.get('custom_answered')):
...
async def process_text_for_pending(update,text):
    ...
    if p.get('awaiting') in ('custom_name','custom_confirm','player_mismatch'):
        if p.get('awaiting')=='custom_confirm':
            ...
            if t in ('no','no gracias'):
                p['awaiting']=None; p['custom_answered']=True; save_state(update.effective_chat.id,state)
    ...
    if parsed.get('size') and p.get('catalog'):
        await update.message.reply_text(item_summary(p)+'\n\nConfirma el artículo:',reply_markup=kb_confirm(p)); save_state(update.effective_chat.id,state); return True
    save_state(update.effective_chat.id,state)
...
async def ping(update,context): await update.message.reply_text(f'V8.3 activo. Vision: {"OK" if OPENAI_API_KEY else "FALTA OPENAI_API_KEY"} ({VISION_MODEL}). Excel: {"OK" if EXCEL_FILE.exists() else "FALTA"}.')
...
def health():
    ...
    return f'Bot V8.1 activo - Excel: {"OK" if exists else "FALTA"} - productos: {n} - vision: {"OK" if OPENAI_API_KEY else "FALTA OPENAI_API_KEY"}',200 if exists else 503
...
def main():
    ...
    print(f'V8.1 listo | Excel={EXCEL_FILE.name} | productos={len(catalog_rows())} | Vision={VISION_MODEL} | API_KEY={"OK" if OPENAI_API_KEY else "FALTA"}', flush=True)
