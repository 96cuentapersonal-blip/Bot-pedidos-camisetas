...
def append_pedido_excel(order_id, update, item, confidence):
    """Guarda un artículo confirmado en la hoja PEDIDOS del Excel maestro."""
    if not os.path.exists(EXCEL_FILE):
        raise FileNotFoundError(f"No existe {EXCEL_FILE}")

    wb = load_workbook(EXCEL_FILE)
    if "PEDIDOS" not in wb.sheetnames:
        wb.close()
        raise KeyError("La hoja PEDIDOS no existe en el Excel maestro")

    ws = wb["PEDIDOS"]
    headers = [str(c.value).strip() if c.value is not None else "" for c in ws[1]]
    col = {h: i + 1 for i, h in enumerate(headers) if h}

    required = ["PEDIDO_ID", "FECHA", "CLIENTE", "ESTADO", "ID_PRODUCTO",
                "MODELO", "DESCRIPCION", "TALLA", "CANTIDAD", "NOMBRE",
                "NUMERO", "PARCHE", "IMAGEN_REFERENCIA", "NOTAS", "ORIGEN", "CONFIANZA"]
    missing = [h for h in required if h not in col]
    if missing:
        wb.close()
        raise KeyError("Faltan columnas en PEDIDOS: " + ", ".join(missing))

    catalog = item.get("catalog") or {}
    team, size, player, number, patch, edition = parse_request(item.get("text", ""))
    user = update.effective_user
    if user:
        client = user.username or user.full_name or str(user.id)
    else:
        client = "Telegram"
    ...
    values = {
        ...
        "CLIENTE": client,
        "ESTADO": "ABIERTO",
        "ID_PRODUCTO": catalog.get("ID_PRODUCTO"),
        "MODELO": catalog.get("MODELO"),
        "DESCRIPCION": candidate_text(catalog),
        "TALLA": size or item.get("size") or "",
        "CANTIDAD": item.get("cantidad", 1),
        "NOMBRE": catalog.get("JUGADOR") or player or "",
        "NUMERO": catalog.get("NUMERO") or number or "",
        "PARCHE": catalog.get("PARCHE") or patch or "",
        "IMAGEN_REFERENCIA": catalog.get("IMAGEN") or "",
        "NOTAS": item.get("photo_caption") or "",
        "ORIGEN": "Telegram",
...
def rebuild_pedido_proveedor(order_id):
    """Reconstruye la hoja PEDIDO_PROVEEDOR para el pedido indicado.
    Cada camiseta ocupa una fila; si CANTIDAD > 1 se repite la fila.
    Si CATALOGO.IMAGEN apunta a un archivo local existente, se inserta la imagen.
    """
    if not os.path.exists(EXCEL_FILE):
        raise FileNotFoundError(f"No existe {EXCEL_FILE}")
    wb = load_workbook(EXCEL_FILE)
    if "PEDIDOS" not in wb.sheetnames or "PEDIDO_PROVEEDOR" not in wb.sheetnames:
        wb.close()
        raise KeyError("Falta PEDIDOS o PEDIDO_PROVEEDOR en el Excel maestro")
    wp = wb["PEDIDO_PROVEEDOR"]
    wo = wb["PEDIDOS"]

    # Limpiar salida anterior, conservando encabezados/anchos/estilos del template.
    for row in range(2, wp.max_row + 1):
        for col in range(1, wp.max_column + 1):
            wp.cell(row=row, column=col).value = None
    wp._images = []

    headers = [str(c.value).strip() if c.value is not None else "" for c in wo[1]]
    idx = {h:i+1 for i,h in enumerate(headers) if h}
    required = ["PEDIDO_ID","ID_PRODUCTO","MODELO","DESCRIPCION","TALLA","CANTIDAD","NOMBRE","NUMERO","PARCHE","IMAGEN_REFERENCIA","ESTADO"]
    if any(x not in idx for x in required):
        wb.close(); raise KeyError("Faltan columnas necesarias en PEDIDOS")

    rows = []
    for r in range(2, wo.max_row + 1):
        pid = wo.cell(r, idx["PEDIDO_ID"]).value
        estado = str(wo.cell(r, idx["ESTADO"]).value or "").upper()
        if str(pid) != str(order_id) or estado in ("ANULADO", "CANCELADO"):
            continue
        qty = wo.cell(r, idx["CANTIDAD"]).value or 1
        try: qty = max(1, int(qty))
        except Exception: qty = 1
        for _ in range(qty): rows.append(r)

    # Salida compatible con el formato existente: imagen en A, descripción en B, talla en C.
    for out_row, src_row in enumerate(rows, start=2):
        model = wo.cell(src_row, idx["MODELO"]).value or ""
        desc = wo.cell(src_row, idx["DESCRIPCION"]).value or ""
        size = wo.cell(src_row, idx["TALLA"]).value or ""
        name = wo.cell(src_row, idx["NOMBRE"]).value or ""
        num = wo.cell(src_row, idx["NUMERO"]).value or ""
        patch = wo.cell(src_row, idx["PARCHE"]).value or ""
        # B muestra una descripción útil para proveedor.
        extra=[]
        if name: extra.append(str(name))
        if num: extra.append(f"#{num}")
        if patch: extra.append(f"Parche {patch}")
        desc_out = str(desc)
        if extra and not all(str(x).lower() in desc_out.lower() for x in extra):
            desc_out += " — " + " ".join(extra)
        wp.cell(out_row, 1).value = model
        wp.cell(out_row, 2).value = desc_out
        wp.cell(out_row, 3).value = size
        wp.row_dimensions[out_row].height = 95

        img_ref = wo.cell(src_row, idx["IMAGEN_REFERENCIA"]).value
        if img_ref:
            candidate_paths = [str(img_ref)]
            if not os.path.isabs(str(img_ref)):
                candidate_paths += [os.path.join(os.path.dirname(EXCEL_FILE), str(img_ref)), os.path.join("catalogo_imagenes", str(img_ref))]
            image_path = next((x for x in candidate_paths if os.path.exists(x)), None)
            if image_path:
                try:
                    img = XLImage(image_path)
                    img.width = 95
                    img.height = 95
                    wp.add_image(img, f"A{out_row}")
                except Exception:
                    pass

    # Encabezado de control para el proveedor.
    wp["G1"] = f"PEDIDO #{int(order_id):04d}" if str(order_id).isdigit() else f"PEDIDO {order_id}"
    wp["G2"] = f"Actualizado: {datetime.now().strftime('%Y-%m-%d %H:%M')}"
    wb.save(EXCEL_FILE)
    wb.close()


def cancel_last_pedido_excel(order_id):
    """Marca como ANULADO el último registro del pedido para que no llegue al proveedor."""
    if not os.path.exists(EXCEL_FILE): return
    wb=load_workbook(EXCEL_FILE)
    if "PEDIDOS" not in wb.sheetnames: wb.close(); return
    ws=wb["PEDIDOS"]
    headers=[str(c.value).strip() if c.value is not None else "" for c in ws[1]]
    idx={h:i+1 for i,h in enumerate(headers) if h}
    if "PEDIDO_ID" not in idx or "ESTADO" not in idx: wb.close(); return
    for r in range(ws.max_row,1,-1):
        if str(ws.cell(r,idx["PEDIDO_ID"]).value)==str(order_id) and str(ws.cell(r,idx["ESTADO"]).value or "").upper() not in ("ANULADO","CANCELADO"):
            ws.cell(r,idx["ESTADO"]).value="ANULADO"
            break
...
async def handle_message(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if low in ("cierro pedido", "/cerrar"):
        ...
        try:
            rebuild_pedido_proveedor(order["id"])
        except Exception as e:
            await update.message.reply_text(f"⚠️ Pedido cerrado, pero no pude actualizar PEDIDO_PROVEEDOR: {e}")
            return
        order["status"] = "CERRADO"
        await update.message.reply_text(f"🔴 Pedido #{order['id']} cerrado.\nTotal de artículos: {len(order['items'])}\n\n📋 PEDIDO_PROVEEDOR actualizado en el Excel.")
        return
    ...
    if low in ("corrige la última", "corrige la ultima", "editar última", "editar la última"):
        if order["items"]:
            order["items"].pop()
            try:
                cancel_last_pedido_excel(order["id"])
                rebuild_pedido_proveedor(order["id"])
            except Exception:
                pass
    ...
    if pending_candidates and low in ("1", "2", "3"):
        ...
        if selected_index < len(pending_candidates):
            ...
            confidence = pending_candidates[selected_index][0]
            try:
                append_pedido_excel(order["id"], update, item, confidence)
                rebuild_pedido_proveedor(order["id"])
            except Exception as e:
                item["status"] = "ERROR_EXCEL"
                await update.message.reply_text(f"⚠️ Producto identificado, pero no pude guardarlo en Excel: {e}")
                return
    ...
    if pending_candidates and re.fullmatch(r"[A-Za-z0-9_-]+", text):
        ...
        for _, candidate in pending_candidates:
            if norm(candidate.get("ID_PRODUCTO")) == wanted:
                ...
                item["catalog"] = candidate
                item["status"] = "IDENTIFICADO"
                confidence = next((score for score, cand in pending_candidates if cand is candidate), 0)
                try:
                    append_pedido_excel(order["id"], update, item, confidence)
                    rebuild_pedido_proveedor(order["id"])
                except Exception as e:
    ...
    if len(candidates) == 1 and candidates[0][0] >= 60:
        ...
        try:
            append_pedido_excel(order["id"], update, item, candidates[0][0])
            rebuild_pedido_proveedor(order["id"])
        except Exception as e:
            item["status"] = "ERROR_EXCEL"
            await update.message.reply_text(f"⚠️ Producto identificado, pero no pude guardarlo en Excel: {e}")
...
def main():
    ...
    print("Bot V3.3 iniciado."); application.run_polling(allowed_updates=Update.ALL_TYPES)
