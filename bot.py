...
EXCEL_NAME = os.environ.get('EXCEL_FILE', 'SISTEMA_PEDIDOS_SERGIO_V7.xlsx')
EXCEL_FILE = Path(EXCEL_NAME)
if not EXCEL_FILE.is_absolute():
    EXCEL_FILE = BASE_DIR / EXCEL_FILE
DB_NAME = os.environ.get('DB_FILE', 'bot_v7.sqlite3')
...
# V7: vision + catalog + official roster order assistant.
...
async def close_order(update,context):
    ...
    await update.message.reply_text('¿Qué quieres hacer ahora?', reply_markup=InlineKeyboardMarkup([[cb('📋 Revisar pedido','review'),cb('🆕 Siguiente pedido','new_order_btn')]]))
...
async def handle_photo(update,context):
    ...
    if vis:
        ...
        if filtered:
            filtered.sort(key=lambda x:x[0],reverse=True); p['candidates']=[r for _,r in filtered]
            # Auto-select only when vision confidence is high and the catalog has one clear product.
            if p['confidence']>=90 and len(filtered)==1:
                p['catalog']=filtered[0][1]
            elif p['confidence']>=90 and len(filtered)>1:
                top=filtered[0][0]; second=filtered[1][0]
                if top-second>=18: p['catalog']=filtered[0][1]
        else:
    ...
    if p.get('catalog'):
        ...
        return
    if p.get('candidates'):
        await update.message.reply_text('📸 **Imagen analizada.** Encontré estas coincidencias del CATALOGO:',reply_markup=candidate_keyboard([(score_row(r,p['parsed'],caption),r) for r in p['candidates']]),parse_mode='Markdown')
    elif vis:
...
async def callbacks(update,context):
    ...
    if data=='new_order_btn':
        ...
        ns={'id':next_id(),'status':'ABIERTO','items':[],'pending':None}; save_state(q.message.chat.id,ns)
        await q.message.reply_text(f'🟢 Pedido #{ns["id"]:04d} abierto. Envía la primera foto.')
        return
    if data=='close_now':
        # Reuse the same close logic without requiring a text command.
        if not state.get('items'):
            await q.message.reply_text('⚠️ El pedido está vacío.'); return
        set_order_status(state['id'],'CERRADO'); state['status']='CERRADO'; save_state(q.message.chat.id,state)
        try: path=rebuild_output(state['id'])
        except Exception as e:
            await q.message.reply_text(f'⚠️ No pude generar el Excel: {e}'); return
        await q.message.reply_text(f'🔒 Pedido #{state["id"]:04d} cerrado.\n📦 Artículos: {len(state["items"])}')
        with open(path,'rb') as f: await q.message.reply_document(document=f,filename=os.path.basename(path),caption=f'📄 PEDIDO_PROVEEDOR_{state["id"]:04d}.xlsx')
        return
    if data=='review':
        lines=[f'📦 Pedido #{state["id"]:04d} — {state["status"]}','']
        for i,x in enumerate(state['items'],1):
...
async def ping(update,context): await update.message.reply_text(f'V7 activo. Vision: {"OK" if OPENAI_API_KEY else "FALTA OPENAI_API_KEY"}. Excel: {"OK" if EXCEL_FILE.exists() else "FALTA"}.')
...
def health():
    ...
    return f'Bot V7 activo - Excel: {"OK" if exists else "FALTA"} - productos: {n} - vision: {"OK" if OPENAI_API_KEY else "FALTA OPENAI_API_KEY"}',200 if exists else 503
