...
# V9: stable Telegram callbacks, deterministic vision retry, manual fallback, player aliases, single-photo quantity=1, transactional close.
...
async def close_order(update,context):
    ...
    try:
        ...
        try:
            state['status']='ABIERTO'; save_state(chat,state); set_order_status(state['id'],'ABIERTO')
        except Exception: pass
...
async def process_text_for_pending(update,text):
    ...
    if p.get('awaiting') in ('custom_name','custom_confirm','player_mismatch','manual_data'):
        if p.get('awaiting')=='manual_data':
            # Fall through to the normal merge/parser below; explicit manual text has priority.
            p['awaiting']=None
...
async def callbacks(update,context):
    ...
    if data=='manual:write':
        p['awaiting']='manual_data'; state['pending']=p; save_state(q.message.chat.id,state)
    ...
    if data=='close_now':
        ...
        try:
            ...
            if not path or not os.path.exists(path): raise RuntimeError('No se generó el Excel final.')
            set_order_status(state['id'],'CERRADO')
            state['status']='CERRADO'; save_state(q.message.chat.id,state)
            ...
            try:
                state['status']='ABIERTO'; save_state(q.message.chat.id,state); set_order_status(state['id'],'ABIERTO')
            except Exception: pass
...
def main():
    ...
    print(f'V9 listo | Excel={EXCEL_FILE.name} | productos={len(catalog_rows())} | Vision={VISION_MODEL} | API_KEY={"OK" if OPENAI_API_KEY else "FALTA"}', flush=True)
